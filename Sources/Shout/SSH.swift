...
public class SSH {
    ...
    public func execute(_ command: String, output: ((_ output: String) -> ())) throws -> Int32 {
        ...
        while dataLeft {
            ...
            case .data(let data):
                let str: String = String(data: data, encoding: .utf8) ?? String(data: data, encoding: .nonLossyASCII)
                output(str)
            case .done:
                dataLeft = false
            case .eagain:
    ...
    public func sendFile(localURL: URL, remotePath: String, permissions: FilePermissions = .default) throws -> Int32 {
        ...
        while inputStream.hasBytesAvailable {
            let bytesRead = buffer.withUnsafeMutableBytes { data in
                inputStream.read(data.bindMemory(to: UInt8.self).baseAddress!, maxLength: bufferSize)
            }
